...
ORBIT_RADIUS_M="${ORBIT_RADIUS_M:-260}"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT="${DATASET_OUTPUT:-$DATASET_ROOT/ucc_fixedwing_kau_400m_$STAMP}"
...
if ! gz topic -l 2>/dev/null | grep -q '^/ucc/fixed_wing/kinematics$'; then
  echo "[ERROR] Gazebo fixed-wing kinematics topic is unavailable." >&2
  exit 1
fi

if ! pgrep -f '[g]azebo_airsim_bridge.py' >/dev/null; then
  echo "[ERROR] Gazebo-to-AirSim bridge is not running." >&2
  echo "        Run 01_prepare_takeoff.sh first." >&2
  exit 1
fi
...
if not math.isclose(float(vehicle.get("Z", float("nan"))), -400.0, abs_tol=1e-6):
    raise SystemExit("[ERROR] AirSim runtime profile is not the KAU 400 m profile")
...
altitude_m = 400.0 - float(second.z_val)
if distance < 5.0:
    raise SystemExit(
        f"[ERROR] AirSim aircraft moved only {distance:.2f} m in 1 s; "
        "Gazebo motion is not being injected"
    )
if not 350.0 <= altitude_m <= 450.0:
    raise SystemExit(
        f"[ERROR] current local altitude is {altitude_m:.1f} m; "
        "restart the bridge after prepare takeoff"
    )
print(
    f"[PREFLIGHT] Gazebo live motion={distance:.1f} m/s-equivalent, "
...
"$VENV/bin/python" -u "$MISSION" kau_figure8 \
  --duration-sec "$DURATION_SEC" \
  --speed-mps "$SPEED_MPS" \
  --bank-limit-deg "$BANK_LIMIT_DEG" \
  --orbit-radius-m "$ORBIT_RADIUS_M" \
...
echo "[KAU_FIXEDWING_DATASET_COMPLETE] $OUTPUT"
